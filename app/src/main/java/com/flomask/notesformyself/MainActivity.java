...
import com.flomask.notesformyself.Adapters.NotesListAdapter;
import com.flomask.notesformyself.Database.RoomDB;
...
public class MainActivity extends AppCompatActivity implements PopupMenu.OnMenuItemClickListener{
    ...
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        if(requestCode == 101) {
            ...
            if(resultCode == Activity.RESULT_OK){
                Notes editingNote = (Notes) data.getSerializableExtra("note");
                database.mainDAO().update(editingNote.getID(), editingNote.getTitle(), editingNote.getNotes());
                notes.clear();
                notes.addAll(database.mainDAO().getAll());
                notesListAdapter.notifyDataSetChanged();
            }
        ...
        switch (menuItem.getItemId()){
            case R.id.pin:
                if(selectedNote.isPinned()){
                    ...
                    Toast.makeText(MainActivity.this, "Desafixado!", Toast.LENGTH_SHORT).show();
                } else {
